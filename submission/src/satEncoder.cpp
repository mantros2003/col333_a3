...
void basicSATGen(int N, int M, int K, int J, const vector<MetroLine>& metroLines, const vector<Point>& popularCells, const string& filename) {
        ...
        for (int r = 0; r < M; ++r) {
            for (int c = 0; c < N; ++c) {
                ...
                for (int in_edge: incoming_edges) {
                    clauses.push_back({-in_edge, occ_var});
                    backward_clause.push_back(in_edge);
                }
                for (int out_edge: outgoing_edges) {
                    clauses.push_back({-out_edge, occ_var});
                    backward_clause.push_back(out_edge);
                }
                // Edge => Occupied
                clauses.push_back(backward_clause);
                ...
                // Cyclicity not allowed
                // Case 1: Came from UP (r-1, c), cannot exit going UP
                if (r > 0) {
                    int in_from_up = get_edge_var(k, r - 1, c, DOWN);
                    int out_to_up = get_edge_var(k, r, c, UP);
                    clauses.push_back({-in_from_up, -out_to_up});
                }

                // Case 2: Came from DOWN (r+1, c), cannot exit going DOWN
                if (r < M - 1) {
                    int in_from_down = get_edge_var(k, r + 1, c, UP);
                    int out_to_down = get_edge_var(k, r, c, DOWN);
                    clauses.push_back({-in_from_down, -out_to_down});
                }

                // Case 3: Came from LEFT (r, c-1), cannot exit going LEFT
                if (c > 0) {
                    int in_from_left = get_edge_var(k, r, c - 1, RIGHT);
                    int out_to_left = get_edge_var(k, r, c, LEFT);
                    clauses.push_back({-in_from_left, -out_to_left});
                }

                // Case 4: Came from RIGHT (r, c+1), cannot exit going RIGHT
                if (c < N - 1) {
                    int in_from_right = get_edge_var(k, r, c + 1, LEFT);
                    int out_to_right = get_edge_var(k, r, c, RIGHT);
                    clauses.push_back({-in_from_right, -out_to_right});
                }
                ...
                clauses.push_back({-turn_var, occ_var});
            }
        }

        // Edge clauses for the corners and edges of the grid
        for (int c = 0; c < N; ++c) {
            clauses.push_back({-get_edge_var(k, 0, c, UP)});
            clauses.push_back({-get_edge_var(k, M-1, c, DOWN)});
        }
        for (int r = 0; r < M; ++r) {
            clauses.push_back({-get_edge_var(k, r, 0, LEFT)});
            clauses.push_back({-get_edge_var(k, r, N-1, RIGHT)});
        }
    ...
    for (const auto& cell: popularCells) {
        cout << "Encoding popular cells" << endl;
        vector<int> popular_clause;
        for (int k = 0; k < K; ++k) {
            popular_clause.push_back(get_occ_var(k, cell.y, cell.x));
        }
        if (!popular_clause.empty()) { clauses.push_back(popular_clause); }
    }

    // No two cells are occupied at the same time
    for (int r = 0; r < M; ++r) {
        for (int c = 0; c < N; ++c) {
